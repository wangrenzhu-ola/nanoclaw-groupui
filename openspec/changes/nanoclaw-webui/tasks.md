...
- [x] **Task 1.2: 数据库与 Prisma 设置** <!-- id: 1.2 -->
...
- [x] **Task 2.1: 布局与导航** <!-- id: 2.1 -->
  - **描述**: 创建包含响应式侧边栏的 App Shell 布局。实现从 API 获取数据的“私聊”和“群组”列表。
  - **前置任务**: Task 1.4
  - **验收标准**: 侧边栏显示 Agent/群组。聊天间导航正常工作。移动端响应式正常。
- [x] **Task 2.2: 聊天控制台 UI** <!-- id: 2.2 -->
  - **描述**: 构建主聊天界面。实现带自动滚动的消息列表。构建带自动扩展文本框和图片上传按钮的输入区域。
  - **前置任务**: Task 2.1
  - **验收标准**: 消息正确渲染（用户右侧，Agent 左侧）。Markdown 渲染正常。输入区域可处理文本和图片。
- [x] **Task 2.3: 实时聊天集成** <!-- id: 2.3 -->
  - **描述**: 将聊天控制台连接到 Socket.io。处理 `agent:response` 以实现流式文本（打字机效果）。实现已发送消息的乐观更新。
  - **前置任务**: Task 1.6, Task 2.2
  - **验收标准**: 消息即时显示。Agent 回复逐字流式显示。
- [x] **Task 2.4: Agent 资料页 (旧)** <!-- id: 2.4 -->
  - **描述**: 创建 Agent 详情页面。实现概览、记忆（Markdown 编辑器）、任务、沙箱和安全标签页。
  - **前置任务**: Task 1.4
  - **验收标准**: Agent 详情加载成功。可以编辑并保存记忆。
- [x] **Task 2.5: 群组与提及功能** <!-- id: 2.5 -->
  - **描述**: 在输入区域实现 `@mention` 弹出层。添加“创建群组”模态框。处理 `@everyone` 高亮显示。
  - **前置任务**: Task 2.3
  - **验收标准**: 输入 `@` 显示 Agent 列表。创建群组后添加到侧边栏。
- [ ] **Task 2.6: Slack 风格交互重构与功能补全** <!-- id: 2.6 -->
  - **描述**: 优化界面交互，使频道（Channel）和 Agent（Member）的详情分离，并修复核心交互漏洞。
    1. **侧边栏与路由**: 侧边栏区分 `Agents` 和 `Channels`。修复创建频道后跳转 404 的问题（添加加载状态或重试机制）。
    2. **频道详情页 (Channel Details)**: 重构详情页，仅展示频道配置。新增 **成员列表 (Members)** 模块。新增 **删除频道 (Delete Channel)** 功能。
    3. **Agent 详情页 (Agent Profile)**: 独立页面展示全局记忆和安全配置。
    4. **成员管理**: 在频道详情页实现邀请/移除 Agent 的完整交互。
    5. **@提及优化**: 仅当频道内有成员时才启用 `@` 功能，空频道禁用。
    6. **输入态**: 实现 `agent:typing` 事件监听，在聊天界面展示“Agent 正在输入...”提示。
  - **前置任务**: Task 2.4, Task 2.5
  - **验收标准**: 详情页职责分离清晰。创建频道跳转正常。频道可删除。成员可管理。空频道无法@。输入态正常显示。

## 测试与验证 (Testing & Validation)

- [x] **Task 3.1: 单元测试** <!-- id: 3.1 -->
  - **描述**: 使用 `webapp-testing` 技能为工具函数（Markdown 解析器、日期格式化）和 API 路由处理程序编写单元测试。
  - **前置任务**: Task 1.5
  - **验收标准**: 所有单元测试通过。

- [x] **Task 3.2: 集成测试** <!-- id: 3.2 -->
  - **描述**: 测试从 API -> DB -> 文件系统的流程。验证 Socket.io 连接和事件处理。补充 “图片上传与 MiniMax API 交互” 的测试场景。
  - **前置任务**: Task 1.6
  - **验收标准**: 集成测试通过。数据库和文件系统状态一致。图片功能对接无异常。

- [ ] **Task 3.3: 端到端测试与漏洞验收** <!-- id: 3.3 -->
  - **描述**: 使用 Playwright/Cypress 测试完整用户流程，并覆盖所有修复的漏洞。
    1. **基础流程**: 登录 -> 创建频道 -> 添加Agent -> 发送消息 -> 接收真实LLM回复。
    2. **404 修复验证**: 创建频道后自动跳转，确认无 404。
    3. **成员管理验证**: 验证添加/移除成员后，详情页列表和 @提及 列表同步更新。
    4. **空频道约束验证**: 验证在无成员频道输入 `@` 无反应。
    5. **删除功能验证**: 删除频道后，侧边栏消失，数据不再显示。
    6. **输入态验证**: 发送消息后，在收到回复前看到“正在输入...”提示。
  - **前置任务**: Task 2.6
  - **验收标准**: E2E 场景全部通过。6个核心漏洞/需求点全部验证无误。

## 部署与交付 (Deployment & Delivery)

- [x] **Task 4.1: Docker 设置** <!-- id: 4.1 -->
  - **描述**: 为 WebUI 创建 `Dockerfile`。更新 `docker-compose.yml` 以包含 WebUI、Nginx 和 NanoClaw Core，确保共享卷和网络配置正确。修复 Alpine 环境下的 Node 依赖构建。
  - **前置任务**: Task 3.3
  - **验收标准**: `docker-compose up` 启动所有服务。容器间可通信。重启后数据持久化。
- [x] **Task 4.2: 文档编写** <!-- id: 4.2 -->
  - **描述**: 编写 `README.md`（部署指南）和 `USER_GUIDE.md`。包含测试账号凭证。补充docker-compose.yml关键配置说明（包括大模型环境变量）。
  - **前置任务**: Task 4.1
  - **验收标准**: 文档完整准确。指令可验证。
- [x] **Task 4.3: 最终验收** <!-- id: 4.3 -->
  - **描述**: 使用测试账号进行最终人工验证。确保满足 `spec.md` 中的所有需求。确认真实大模型 (MiniMax) 对接顺畅。
  - **前置任务**: Task 4.2
