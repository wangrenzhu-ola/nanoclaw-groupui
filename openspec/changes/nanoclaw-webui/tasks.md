...
| ---- | ---------------------- | ----------- | ---------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| R1   | C-01                   | In Progress | 2026-03-15 | `src/webui-api.ts`、`src/webui-api.test.ts`、`src/r1-acceptance.ts`                                              | 已完成导航/消息读取后端API、可视化验收页与测试，前端Sidebar与路由待完成      |
| R1   | C-02                   | Pending     | 2026-03-15 | -                                                                                                                | 依赖C-01前端导航完成后进入                                                   |
| R1   | C-03                   | Pending     | 2026-03-15 | -                                                                                                                | 依赖C-01并需线程数据模型支持                                                 |
| R2   | C-04A,C-04B,C-04C,C-06 | In Progress | 2026-03-15 | `src/db.ts`、`src/webui-api.ts`、`src/webui-api.test.ts`、`src/credential-proxy.test.ts`、`src/r2-acceptance.ts` | 已落地消息编辑/撤回API、提及解析接口与R2验收页，联通性门禁新增自动化测试基线 |
| R3   | C-07A,C-07B,C-07C,C-09 | In Progress | 2026-03-15 | `src/db.ts`、`src/webui-api.ts`、`src/webui-api.test.ts`、`src/r3-acceptance.ts`、`package.json`                 | 已落地未读聚合/已读标记、在线状态、静音与通知冲突评估接口，前端完整态待补齐  |
