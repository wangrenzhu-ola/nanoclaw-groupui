...
| v3.0 | 2026-03-15 | 执行Agent | 校准R5/R6状态与证据：补充自动化回归结果，标记QA执行留痕待完成 |
| v3.1 | 2026-03-15 | 执行Agent | 完成C-05落地与R5/R6证据回填：新增频道策略/审计API并补齐QA执行留痕 |
| v3.2 | 2026-03-15 | 执行Agent | R5人工验收通过并发起R6验收：任务跟踪中R5切换Accepted |
| v3.3 | 2026-03-15 | 执行Agent | R6验收通过并启动R7迭代：新增R7/R8状态跟踪并冻结部署与合规门禁 |
| v3.4 | 2026-03-15 | 执行Agent | 完成R7首批开发：新增演练与发布检查脚本、部署清单、环境变量模板 |
...
| ---- | ----------------------------- | ----------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| R1   | C-01,C-02,C-03                | Accepted    | 2026-03-15 | `src/db.ts`、`src/webui-api.ts`、`src/webui-api.test.ts`、`src/r1-acceptance.ts`                                                                                                          | 会话导航、DM与线程能力已验收通过                                              |
| R2   | C-04A,C-04B,C-04C,C-06        | Accepted    | 2026-03-15 | `src/db.ts`、`src/webui-api.ts`、`src/webui-api.test.ts`、`src/r2-acceptance.ts`                                                                                                          | 消息操作、提及规则与并发去重能力已验收通过                                    |
| R3   | C-07A,C-07B,C-07C,C-09        | Accepted    | 2026-03-15 | `src/db.ts`、`src/webui-api.ts`、`src/webui-api.test.ts`、`src/r3-acceptance.ts`、`package.json`                                                                                          | 未读/在线/静音/通知冲突能力已验收通过                                         |
| R4   | C-08A,C-08B,C-08C             | Accepted    | 2026-03-15 | `src/db.ts`、`src/webui-api.ts`、`src/webui-api.test.ts`、`src/r4-acceptance.ts`、`package.json`                                                                                          | 搜索与文件中心能力已验收通过                                                  |
| R5   | C-05,C-10                     | Accepted    | 2026-03-15 | `src/webui-api.ts`、`src/db.ts`、`src/webui-api.test.ts`、`src/ipc.ts`、`src/sender-allowlist.ts`、`src/ipc-auth.test.ts`、`src/sender-allowlist.test.ts`                                 | C-05频道策略与审计API已落地，C-10边界能力与R5自动化证据已补齐，并通过人工验收 |
| R6   | C-11                          | Accepted    | 2026-03-15 | `src/group-queue.ts`、`src/container-runner.ts`、`src/credential-proxy.ts`、`src/group-queue.test.ts`、`src/container-runner.test.ts`、`src/credential-proxy.test.ts`、`qa-test-cases.md` | R6异常恢复与认证拦截自动化通过，QA执行留痕已回填，并通过人工验收              |
| R7   | D-05,E-01                     | Accepted    | 2026-03-15 | `scripts/r7_backup_restore_drill.sh`、`scripts/r7_release_check.sh`、`DEPLOYMENT.md`、`DEPLOYMENT_CHECKLIST.md`                                                                           | R7已验收通过，合规与部署演练完成                                              |
| R8   | D-01,D-02,D-03,D-04,E-02,E-03 | In Progress | 2026-03-15 | `pmo-round-schedule.md`、`qa-test-cases.md`                                                                                                                                               | 进入全量回归与发布验收阶段，QA全面介入                                        |
